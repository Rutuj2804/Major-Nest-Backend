...
export class ChatService {
    ...
    async createRoom(chatRoomDTO: ChatRoomDTO, user: AuthInterface) {
        if (chatRoomDTO.users.length > 1) {
            ...
            newRoom.save();
            newRoom = await newRoom.populate('users');
            return newRoom;
        } else {
            let newRoom = new this.chatModel({
                users: chatRoomDTO.users,
    ...
    async getMessagesOfRooms(id: string) {
        ...
        for (let i = 0; i < rooms.messages.length; i++) {
            await this.messageModel.findByIdAndUpdate(rooms.messages[i]._id, { isRead: true })
        }
        return rooms
    }
    ...
    async sendMessagesToRooms(id: string, text: string, user: string) {
        ...
        const msg = new this.messageModel({ text: text, sender: user })

        await this.chatModel.findByIdAndUpdate(id, { $push: { messages: msg } }, { returnOriginal: false }).populate("users")
        return msg
    }
}
